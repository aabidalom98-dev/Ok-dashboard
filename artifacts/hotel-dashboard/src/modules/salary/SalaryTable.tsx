...
export function SalaryTable() {
  ...
  const [sortKey, setSortKey] = useState<keyof typeof filteredData[0]>('netSalary');
  ...
  const exportCSV = () => {
    const h = ['Employee','Department','Designation','Gross','Net','Bonus','Deductions','PF','ESI','Tax','Month','Status'];
    const rows = sorted.map(r => [r.employeeName, r.department, r.designation, r.grossSalary, r.netSalary, r.bonus, r.deductions, r.pf, r.esi, r.tax, r.payrollMonth, r.paymentStatus]);
    const csv = [h, ...rows].map(r => r.join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv' });
    const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'salary.csv'; a.click();
  };
  ...
  const statusColor = (s: string) => {
    ...
    if (sl.includes('paid') || sl.includes('complet')) return 'bg-emerald-500/15 text-emerald-500';
    if (sl.includes('pend')) return 'bg-amber-500/15 text-amber-500';
  ...
  return (
    <div className="bg-card border border-border rounded-2xl overflow-hidden">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 p-5 border-b border-border">
        <h3 className="text-sm font-semibold text-foreground uppercase tracking-wider">Salary Records</h3>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead className="bg-muted/30">
            <tr>
              {([['employeeName','Employee'], ['department','Dept'], ['designation','Role'], ['grossSalary','Gross'], ['netSalary','Net'], ['bonus','Bonus'], ['deductions','Deductions'], ['pf','PF'], ['tax','Tax'], ['payrollMonth','Month'], ['paymentStatus','Status']] as [keyof typeof filteredData[0], string][]).map(([k, label]) => (
          ...
          <tbody className="divide-y divide-border">
            {pageData.map((r, i) => (
              <tr key={i} className="hover:bg-muted/20 transition-colors">
                ...
                <td className="px-4 py-3 font-medium text-emerald-500">{fmt(r.netSalary)}</td>
                <td className="px-4 py-3">{fmt(r.bonus)}</td>
                <td className="px-4 py-3 text-rose-400">{fmt(r.deductions)}</td>
                <td className="px-4 py-3">{fmt(r.pf)}</td>
                <td className="px-4 py-3">{fmt(r.tax)}</td>
                <td className="px-4 py-3 text-muted-foreground">{r.payrollMonth}</td>
                <td className="px-4 py-3">
                  <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${statusColor(r.paymentStatus)}`}>
                    {r.paymentStatus}
                  </span>
                </td>
              </tr>
